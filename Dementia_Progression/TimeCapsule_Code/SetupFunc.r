...
MMSEResp <- function(y, nstate, eta, gradient){# eta = X*beta
	
	# log(sigma^2) is the parameter for inference because a Gaussian prior is meaningful, 
  # and a Gaussian proposal is appropriate.  Recall that all parameters have Gaussian priors and updates.
	...
	if(gradient){
		...
		# Update each of the 7 rows of the 7 gradient arrays.  
		# However, the 7th rows are all zeros so k = 1:6.  
		# Note that the only nonzero row for the jth partial derivative is the jth row, 
		#for j \in {1,...,6}, and each of the rows for the 7th partial derivative have the same form.  
		# That is why there is only one for-loop.
		for(k in 1:6){
