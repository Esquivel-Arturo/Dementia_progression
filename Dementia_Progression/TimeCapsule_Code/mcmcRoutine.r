...
mcmcRoutine <- function(par, fn, cmap, splineParam, prior.means, prior.std, steps, burnin, designMat, 
	...
	# The positive components of 'cmap' correspond to the parameters to be updatded.  
	# The positive components also correspond to the indices of 'par'.
	parGroups_Index = list()
	parGroups_Index[[1]] = cmap[,c(1,4,7)][cmap[,c(1,4,7)]>0] # Parameters for transitions from A- states to A+ states
	parGroups_Index[[2]] = cmap[,c(2,3)][cmap[,c(2,3)]>0] # Parameters for transitions from N- states to N+ states
	parGroups_Index[[3]] = cmap[,c(5,6)][cmap[,c(5,6)]>0] # Parameters for transitions from non-Dem states to Dem states
	parGroups_Index[[4]] = cmap[,8][cmap[,8]>0][1:7] # Parameters for transitions from non-Dem states to Death
	# ONLY INCLUDES C1 AND C2 OF LINEAR SLOPE, MAYBE BECAUSE WITH THE FIRST TWO THE REST CAN BE EXTRAPOLATED
	parGroups_Index[[5]] = cmap[,12:13][cmap[,12:13]>0] # Parameters for transitions from Dem states to Death
	parGroups_Index[[6]] = unique(cmap[,14:20][cmap[,14:20]>0]) # Parameters for MMSE response mean (68:79)
	parGroups_Index[[7]] = cmap[,21:31][cmap[,21:31]>0] # Rest of responses parameters (80:90)
	#----------------------------------------------------------------------------------------------------------- 
