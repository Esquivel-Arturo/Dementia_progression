...
plotCol <- c("#ffa600","#bc5090",'lightblue',"#003f5c","#665191",'firebrick3')
...
pdf(paste0(Dir,'density_MMSE_','.pdf'))
mmse <- useData$mmse[!is.na(useData$mmse)]

hold <- Mclust(2.15*mmse+24.35967, G=6, modelNames='E')$parameters

# Must un-normalize the mmse data.
density1 <- hold$pro[6]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[68]+24.35967,sd=2.15*exp(parEst[79])^.5)
density2 <- hold$pro[5]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[74]+24.35967,sd=2.15*exp(parEst[79])^.5)
density3 <- hold$pro[4]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[75]+24.35967,sd=2.15*exp(parEst[79])^.5)
density4 <- hold$pro[3]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[76]+24.35967,sd=2.15*exp(parEst[79])^.5)
density5 <- hold$pro[2]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[77]+24.35967,sd=2.15*exp(parEst[79])^.5)
density6 <- hold$pro[1]*dnorm(seq(15,32,by=.01),mean=2.15*parEst[78]+24.35967,sd=2.15*exp(parEst[79])^.5)

hist(2.15*mmse+24.35967, freq=FALSE, xlim=c(15,32),main='Distribution of observed MMSE', xlab=NA, ylab=NA,
	 col='lightgrey', border='gray', breaks=30, cex.axis=1.25, cex.main=2)
...
lines( seq(15,32,by=.01), density1 + density2 + density3 + density4 + density5 +density6, col="black", lwd=2, lty=2)
legend('topleft', legend=c('state 1','state 2','state 3','state 4','state 5','state 6','normal mixture density'),
	    col=c(plotCol[6:1],"black"), lwd=3, lty=c(1,1,1,1,1,1,2), cex=1.25, box.lty=0)
dev.off()



pdf(paste0(Dir,'density_pib_','.pdf'))
...
hist(pib, freq=FALSE, xlim=c(1,3.5), main='Observed PIB density estimates', xlab=NA, ylab=NA,
	 col='lightgrey', border='gray', breaks=sqrt(length(pib)), cex.axis=1.25, cex.main=2, xaxt='n')
...
lines( seq(1,3.5,by=.01), density1 + density2, col="black", lwd=2, lty=2)
legend('topright', legend=c('low burden','high burden','normal mixture density'),
	    col=c(plotCol[1:2],"black"), lwd=3, lty=c(1,1,2), cex=1.25, box.lty=0)
dev.off()



pdf(paste0(Dir,'density_thickness_','.pdf'))
...
hist(thickness, freq=FALSE, xlim=c(1.75,3.4), main='Observed Thickness density estimates', xlab=NA, ylab=NA,
	 col='lightgrey', border='gray', breaks=sqrt(length(thickness)), cex.axis=1.25, cex.main=2, xaxt='n')
...
lines( seq(1.75,3.4,by=.01), density1 + density2, col="black", lwd=2, lty=2)
