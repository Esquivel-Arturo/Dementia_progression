 ...
 // likelihood computation 
  for (t in 1:T) {
    ...
    if(t == 1 || ID[t] != ID[t-1]){
      ...
      // update log-like  
      for (n in 1:N) {
        logptemp[n] = logp[n] + categorical_lpmf(y[t] | to_vector(P[n])); // Conteporariness conditional independence, assumption during the simulation
      }
      ...
      for (n in 1:N) {
        ...
        logptemp[n] = logptemp[n] + categorical_lpmf(y[t] | to_vector(P[n])); // Conteporariness conditional independence, assumption during the simulation
      }
