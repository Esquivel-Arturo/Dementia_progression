...
parameters {
  ...
  vector[R] b_1;
  vector[R] b_2;
  ordered[3] mu; // state-dependent distributions parameters
  real<lower=0> sigma;
}
...
model {
  ...
  vector[R] b_1_years[21]; // array with 21 vectors, each to contain b1 times the corresponding integer year
 ...
 // likelihood computation 
  for (t in 1:T) {
    ...
    if(t == 1 || ID[t] != ID[t-1]){
      ...
      // update log-like  
      for (n in 1:3) {
        logptemp[n] = logp[n] + normal_lpdf(obs[t] | mu[n], sigma) + categorical_lpmf(y[t] | to_vector(P[n])); 
      }
      logptemp[4] = logp[4]  + categorical_lpmf(y[t] | to_vector(P[4]));
        ...
        // update log-like  
         for (n in 1:3) {
          ...
          if(y[t]!=4){logptemp[n] = logptemp[n] + normal_lpdf(obs[t] | mu[n], sigma) + categorical_lpmf(y[t] | to_vector(P[n]));}
        }
